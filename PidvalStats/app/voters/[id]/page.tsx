    ...
    <div className="px-4 md:px-12 py-8 max-w-3xl mx-auto">
      ...
      {stats.histogram.some((h: any) => h.count > 0) && (
        <section className="mb-10">
          <div className="eyebrow mb-3">Які оцінки ставить найчастіше</div>
          <div className="flex items-end gap-1.5 h-24 bg-panel rounded-xl px-4 pt-4 pb-2">
            {stats.histogram.map((h: any) => (
              <div key={h.score} className="flex-1 flex flex-col items-center gap-1.5">
                <span className="text-[9px] text-muted font-utility">{h.count || ""}</span>
                <div
                  className="w-full rounded-t transition-all duration-300"
                  style={{
                    height: `${Math.max(4, (h.count / maxHist) * 100)}%`,
